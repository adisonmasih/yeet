...
module.exports = {
  data: new SlashCommandBuilder()
    .setName("hunt")
    .setDescription("Hunt In The Jungle, Cuz Amm.. Ahh.. You Aren't Mingle :)"),
  testOnly: true,
  async execute(interaction, client) {
    let user = interaction.user;
    let item = client.custom.economy.items.get("bow");
    ...
    if (userData.cooldowns?.hunt > Date.now()) {
      let embed = new EmbedBuilder()
        ...
        .setDescription(
          `⏳ The Animals Are Mating Right Now.. Children Will Be Ready In: \`${prettyMs(
            userData.cooldowns.hunt - Date.now(),
    ...
    let rewardAbles = client.custom.economy.items.filter((item) =>
      item?.keywords?.includes("hunt_item")
    );
    ...
    let noneMessages = [
      "Aww Shit.. The Animals Hated Your Perfume!",
      "Bruh.. Animals Got Away! You Are So Lazy..",
      `"You Just Suck..." - **The Animals**`,
      "Nah Man.. Your Luck Sucks",
      "Animal Aren't Easy To Catch.. It Requires Skill...",
    ...
    userData.cooldowns.hunt = Date.now() + 1000 * 35;
    userData.save();
  },
};
